...
class TubeMesh:
    ...
    def to_stl(self, path: pathlib.Path | str, type: str = 'ascii', overwrite=False):
        if not self.capped:
        ...
        {
            'ascii': self._to_ascii_stl,
            'binary': self._to_binary_stl,
        }[type](path)

    def _to_ascii_stl(self, path: pathlib.Path):
        with path.open('w') as out:
            print(f'solid {path.stem} # Generated by FullControlXYZ', file=out)
            for n, vs in zip(self.mesh_normals, self.triangle_points.reshape(-1,9)):
                print(self.format_stl_triangle(n, vs, binary=False), file=out)
            print(f'endsolid {path.stem}', file=out)

    @staticmethod
    def format_stl_triangle(n, vs, binary=True) -> bytes | str:
        if binary:
            attribute_byte_count = 0 # TODO: support facet color options
            return struct.pack('<'+'f'*(3*(1+3))+'H', *n, *vs, attribute_byte_count)
        else:  # ascii
            return '\n'.join((
    ...
    def _to_binary_stl(self, path: pathlib.Path):
        ...
        with path.open('wb') as out:
            ...
            out.write(struct.pack('<I', len(self.triangles)))
            for n, vs in zip(self.mesh_normals, self.triangle_points.reshape(-1,9)):
                out.write(self.format_stl_triangle(n, vs, binary=True))
...
if __name__ == '__main__':
    ...
    meshes[2].to_stl('flow_tube_ascii.stl')
